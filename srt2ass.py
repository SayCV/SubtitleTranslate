...
'''
    ...
    tmp = pattern.sub(
        r'Dialogue: 0,\1,\2,注释,,0,0,0,,{\3\\fad(500,500)\\fs20}', tmp)
    ...
    pattern = re.compile(
        r'\d{1,4}\r\n(\d{1,2}:\d{1,2}:\d{1,2}\.\d{1,3}) ?--> ?(\d{1,2}:\d{1,2}:\d{1,2}\.\d{1,3})\r\n(.+)\r\n([ …!\w\.,?\'\"-:;\[\]\(\)%$@&*\^+~<>]+)\r\n')
    tmp = pattern.sub(lambda x: auto_counts_sub(x, video_type), tmp)
...
def auto_counts_sub(value, video_type):
...
if __name__ == '__main__':
    ...
    srt2ass(input_file, output_file, video_type)
